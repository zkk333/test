...
def log(username,password,res_session):#主要是返回headers头文件
     global  login_log
     #global  res_session
     url="https://ts1.kuaikuaikeji.com/kas/ulogin"
     ...
     res =res_session.post( url=f1.kk_user_app_url3+f1.app登录,headers=headers,verify=False)
...
#print(res.headers)
     ...
     login_log = res_session.post(url=f1.kk_user_app_url3+f1.app登录, headers=headers,verify=False)#verify参数：是否需要ssl证书的验证，false为忽视验证
